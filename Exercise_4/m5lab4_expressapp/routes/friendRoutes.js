...
router.get("/filter", (req, res) => {
  ...
  if (filterLetter) {
    matchingFriends = matchingFriends.filter(
      (friend) => friend.name[0].toUpperCase() === filterLetter.toUpperCase()
    );
    if (matchingFriends.length > 0) {
      // return valid data when the first letter matches
      res.status(200).json(matchingFriends);
    } else {
      // and an error response when there are no matches
      res
        .status(404)
        .json({ error: "No friends with the first letter of " + filterLetter });
      return;
    }
  }
});
...
router.put("/:id", (req, res) => {
  ...
  const found = friends.findIndex((friend) => friend.id === friendId);
  if (found){
    friends[found].gender = updatedFriend.gender
    friends[found].name = updatedFriend.name
    res.status(200).json({
    ...
    res.status(404).json({result: `No friend with id ${friendId} found`})
  }
